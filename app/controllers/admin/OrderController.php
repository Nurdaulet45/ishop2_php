...
class OrderController extends AppController
{
    public function indexAction(){
        ...
        `order`.`status`,`order`.`date`, `order`.`update_at`, 
         `order`.`currency`, `user`.`name`, 
         ROUND(SUM(`order_product`.`price`), 2) AS `sum` FROM `order` JOIN `user` ON `order`.`user_id` = `user`.`id` JOIN `order_product`
    ...
    public function viewAction(){
        ...
        $this->set(compact('order', 'order_id', 'order_products'));
    }
